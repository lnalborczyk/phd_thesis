...
```{r modelcomparison, eval = TRUE, results = "asis", message = FALSE, warning = FALSE}
#comp <- brms::LOO(bmod1, bmod2, bmod3, bmod4, bmod5, cores = parallel::detectCores() )
#ic <- "loo" # names(comp[[1]])[7]
#mat <- matrix(0, nrow = length(comp) - 1, ncol = 2)
#dimnames(mat) <- list(names(comp)[1:length(comp) - 1], c(toupper(ic), "SE") )

#loo1 <- waic(bmod1)
#loo2 <- waic(bmod2)
#comp <- loo::loo_compare(loo1, loo2, simplify = FALSE)

#bmod1 <- add_loo(bmod1)
#bmod2 <- add_loo(bmod2)

comp <- loo_compare(loo(bmod1), loo(bmod2), loo(bmod3), loo(bmod4), loo(bmod5) ) # simplify = FALSE
...
comp3 <- comp2 %>%
    mutate_at(.vars = 2:4, .funs = funs(round(., 2) ) ) %>%
    mutate("right side of the formula" = lapply(.$model,
        function(x) deparse(f_rhs(as.formula(formula(get(x) ) ) ) ) )
        )

colnames(comp3) <- c("Model", "LOOIC", "SE", paste0("$\\Delta$", "LOOIC"), "right side of the formula")
...
source(here("code", "translation.R") )
...
In brief, we found a weak effect of gender on vowel production variability in Indonesian ($\beta =$ `r round(fixef(bmod5)[2, 1], 3)`, $\ 95 \% \ \text{CrI} =$ [`r round(fixef(bmod5)[2, 3], 3)`, `r round(fixef(bmod5)[2, 4], 3)`], $\ \delta_{t} =$ `r round(mean(delta_t$delta), 3)`, $\ 95 \% \ \text{CrI} = [-0.807, 0.097]$), this effect being associated with a large uncertainty (as expressed by the width of the credible interval). This result seems to show that females tend to pronounce vowels with more variability than males, while the variation observed across vowels (as suggested by $\sigma_{\beta_{vowel}}$) suggests that there might exist substantial inter-vowel variability, that should be subsequently properly studied. A follow-up analysis specifically designed to test the effect of gender on each vowel should help better describe inter-vowel variability (we give an example of such an analysis in the [supplementary materials](#suppApp)). 
