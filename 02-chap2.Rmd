...
```{r setupCH2, echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}
...
```{r facial, out.width = "50%", fig.cap = "Illustration of a facial muscles of interest in the present work. Figure from Patrick J. Lynch, medical illustrator, http://patricklynch.net."}
knitr::include_graphics("assets/facial_muscles.jpg")
```
...
```{r muscle, out.width = "50%", fig.cap = "Illustration of a muscle fiber. Figure from the OpenStax \\textit{Anatomy and Physiology} Textbook. Download for free at http://cnx.org/contents/14fb4ad7-39a1-4eee-ab6e-3ef2482e3e22@15.1."}
knitr::include_graphics("assets/muscle.jpg")
```
...
```{r mfap, out.width = "75%", fig.cap = "Time course of a motor action potential (Figure from Kamen \\& Gabriel, 2010)."}
knitr::include_graphics("assets/mfap.png")
```
...
```{r muap, out.width = "75%", fig.cap = "Motor unit action potential representation (Figure from De Luca et al., 2006)."}
...
```{r emgplot1, fig.width = 6, fig.height = 4, fig.cap = "Simulated EMG signal."}
...
ma <- envelope(emg, method = "MA", wsize = 200)
rms <- envelope(emg, method = "RMS", wsize = 200, units = "samples")

ts.plot(
  emg$values, xlab = "Time (samples)", ylab = "Raw EMG signal (mV)",
  col = "steelblue"
  )
```
...
```{r emgplot2, fig.width = 6, fig.height = 4, fig.cap = "Rectified EMG signal."}
rectified_emg <- abs(emg$values - mean(emg$values) )

ts.plot(
  rectified_emg, xlab = "Time (samples)", ylab = "Rectified EMG signal",
  col = "steelblue"
  )
```
...
```{r emgplot3, fig.width = 6, fig.height = 4, fig.cap = "Illustration of the MAV (in orange) and RMS (in green) values. These two features are usually highly correlated but differ in magnitude. More precisely, the RMS is proportional to the MAV when the signal has a Gaussian shape."}
ts.plot(
  rectified_emg, xlab = "Time (samples)", ylab = "Rectified EMG signal",
  col = "steelblue"
  )

plot(ma, col = "orangered", lwd = 2, add = TRUE)
plot(rms, col = "darkgreen", lwd = 2, add = TRUE)
```
