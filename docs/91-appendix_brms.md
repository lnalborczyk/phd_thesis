...
<caption>(\#tab:modelcomparison)</caption>

<caption>*Model comparison with LOOIC.*</caption>



 Model     LOOIC       SE      $\Delta$LOOIC                                        right side of the formula
-------  ----------  -------  ---------------  --------------------------------------------------------------
 bmod5    -3590.84    68.11        0.00         gender + (1 | subj) + (1 + gender | vowel) + (1 | subj:vowel)
 bmod4    -3536.60    66.88        54.24                           gender + (1 | subj) + (1 + gender | vowel)
 bmod3    -3477.32    67.10       113.52                                    gender + (1 | subj) + (1 | vowel)
 bmod2    -3114.12    65.25       476.71                                                  gender + (1 | subj)
 bmod1    -3101.07    66.78       489.76                                                               gender
...
In brief, we found a weak effect of gender on vowel production variability in Indonesian ($\beta =$ -0.042, $\ 95 \% \ \text{CrI} =$ [-0.1, 0.017], $\ \delta_{t} =$ -0.343, $\ 95 \% \ \text{CrI} = [-0.81, 0.10]$), this effect being associated with a large uncertainty (as expressed by the width of the credible interval). This result seems to show that females tend to pronounce vowels with more variability than males, while the variation observed across vowels (as suggested by $\sigma_{\beta_{vowel}}$) suggests that there might exist substantial inter-vowel variability, that should be subsequently properly studied. A follow-up analysis specifically designed to test the effect of gender on each vowel should help better describe inter-vowel variability (we give an example of such an analysis in the [supplementary materials](#suppApp)). 
