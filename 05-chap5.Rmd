...
As a result of the above sampling procedure, a total of `r n_distinct(df$participant)` French-speaking undergraduate students in Psychology from the Univ. Grenoble Alpes took part in this experiment, in exchange for course credits^[NB: this procedure did not work optimally because we later spotted a mistake in the EMG signal processing workflow. Thus, the sequential testing stopped earlier than it should have.]. They were recruited via mailing list, online student groups, and posters. Each participant provided a written consent and the present study was approved by the local ethics committee.
...
apa_table(
    ...
    note = "*Because the production of /ʒiʒi/ requires a protrusion of the lips, this item theoretically slightly deviates from other items of this class.",
    small = FALSE,
    escape = TRUE
    )
```
...
load("data/ch5/bmod1.rda")
```

```{r outputbmod1, results = "asis"}
results_bmod1 <- 
...
```{r predbmod1, fig.pos = "H", fig.align = "center", fig.width = 7, fig.height = 6, fig.cap = "Raw data along with posterior predictions of the first model for the OOI and the ZYG muscles. Dots represent the mean prediction of this model by condition, while the vertical error bars represent the 95\\% credible intervals around the mean."}
...
```{r ppcbmod1, fig.pos = "H", fig.align = "center", fig.width = 7, fig.height = 4, fig.cap = "Posterior predictive checking for the first model concerning the OOI and ZYG muscles. The dark blue line represents the distribution of the raw data (across all conditions) while light blue lines are dataset generated from the posterior distribution."}
...
p2 <- c(
    prior(normal(0, 10), class = b, resp = "OOI"),
    prior(cauchy(0, 10), class = sigma, resp = "OOI"),
    prior(normal(0, 100), class = alpha, resp = "OOI"),
    prior(normal(0, 10), class = b, resp = "ZYG"),
    prior(cauchy(0, 10), class = sigma, resp = "ZYG"),
    prior(normal(0, 100), class = alpha, resp = "ZYG"),
    prior(normal(0, 10), class = b, resp = "FRO"),
    prior(cauchy(0, 10), class = sigma, resp = "FRO"),
    prior(normal(0, 100), class = alpha, resp = "FRO"),
    prior(normal(0, 10), class = b, resp = "COR"),
    prior(cauchy(0, 10), class = sigma, resp = "COR"),
    prior(normal(0, 100), class = alpha, resp = "COR"),
    prior(normal(0, 10), class = b, resp = "FCR"),
    prior(cauchy(0, 10), class = sigma, resp = "FCR"),
    prior(normal(0, 100), class = alpha, resp = "FCR")
    )

# fitting the skew normal model
# 
# bmod2 <- brm(
#     cbind(OOI, ZYG, FRO, COR, FCR) ~ 0 + condition * item_c, #+ (0 + condition | participant),
#     family = skew_normal(),
#     prior = p2,
#     data = df,
#     sample_prior = TRUE,
...
#save(bmod2, file = "bmod2.rda")
load("data/ch5/bmod2.rda")
bmod2 <- bmod1_skew
#ic_models <- waic(bmod1, bmod2, cores = 4)
```

```{r ppcbmod2, fig.pos = "H", fig.align = "center", fig.width = 7, fig.height = 4, fig.cap = "Posterior predictive checking for the second model concerning the OOI and ZYG muscles. The dark blue line represents the distribution of the raw data while light blue lines are dataset generated from the posterior distribution."}
ppc_ooi <- pp_check(bmod2, resp = "OOI", nsamples = 1e2) +
    theme_minimal(base_size = 11) +
    # hrbrthemes::theme_ipsum(
    #     base_family = "Helvetica", base_size = 12,
    #     axis_title_size = 12, axis_text_size = 10
...
ppc_zyg <- pp_check(bmod2, resp = "ZYG", nsamples = 1e2) +
    theme_minimal(base_size = 11) +
    # hrbrthemes::theme_ipsum(
    #     base_family = "Helvetica", base_size = 12,
    #     axis_title_size = 12, axis_text_size = 10
...
apa_table(
    ...
    placement = "H",
    # align = c("c", "c"),
    caption = "Estimates from the Skew-normal BMLM concerning the OOI and the ZYG.",
    note = "For each muscle (response), the first three lines represent the estimated average
amplitude in each condition, and its standard error (SE).
The three subsequent rows represent the estimated average difference between the two classes
of nonwords in each condition (i.e., the interaction effect). The 'Lower' and 'Upper' columns
...
According to this second model, the EMG amplitude of the OOI was higher than baseline (the standardised score was above zero) in every condition whereas it was only the case in the overt speech condition for the ZYG. We did not observed the hypothesised difference according to the class of nonwords on the OOI during inner speech production (`r glue_data(results_bmod2 %>% filter(Response == "OOI", Term == "Inner Speech x Class"), "$\\beta$ = {Estimate}, 95% CrI [{Lower}, {Upper}], BF~01~ = {BF01}")`) nor on the ZYG (`r glue_data(results_bmod2 %>% filter(Response == "ZYG", Term == "Inner Speech x Class"), "$\\beta$ = {Estimate}, 95% CrI [{Lower}, {Upper}], BF~01~ = {BF01}")`).

```{r predbmod2, fig.pos = "H", fig.align = "center", fig.width = 7, fig.height = 6, fig.cap = "Raw data along with posterior predictions of the second model for the OOI and the ZYG muscles. Dots represent the mean prediction of this model by condition (concerning the location parameter) whereas the vertical error bars represent the 95\\% credible intervals."}
# plotting posterior predictions (for the mean)
...
fitted_values <- fitted(bmod2, newdata = new_data, re_formula = NA)
...
```{r ppcbmod3, fig.pos = "H", fig.align = "center", fig.width = 7, fig.height = 4, fig.cap = "Posterior predictive checking for the Skew-Normal model concerning the OOI and ZYG muscles. The dark blue line represents the distribution of the raw data while light blue lines are dataset generated from the posterior distribution."}
...
```{r predbmod3, fig.pos = "H", fig.align = "center", fig.width = 7, fig.height = 6, fig.cap = "Raw data along with posterior predictions of the third model for the OOI and the ZYG muscles. Dots represent the mean prediction of this model by condition (concerning the location parameter) whereas the vertical error bars represent the 95\\% credible intervals."}
...
```{r 2Demg, fig.pos = "H", dev = "cairo_pdf", fig.align = "center", fig.width = 7, fig.height = 6, fig.cap = "Average EMG amplitude for each nonword in each condition, in the two-dimensional space formed by the OOI and ZYG muscles."}
...
df %>%
    ...
    stat_ellipse(geom = "polygon", alpha = 0.5, show.legend = FALSE) +
    # plotting each word
    geom_point(pch = 21, colour = "black", size = 2, alpha = 0.8, show.legend = FALSE) +
    geom_label_repel(
        aes(label = mot), force = 10,
        fontface = "bold", color = "white",
        segment.colour = "black", segment.alpha = 0.5, show.legend = FALSE
        ) +
...
```{r VarImpOvert, fig.pos = "H", fig.align = "center", fig.width = 4, fig.height = 4, fig.cap = "Scaled variable importance for overt speech classification."}
...
summary_eskes <-
    ...
    arrange(Muscle)
    # mutate(SE = as.numeric(SD) / sqrt(as.numeric(Observations) ) )
