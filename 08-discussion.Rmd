...
```{r, eval = FALSE}
# Reproducing fig in Gershman (2019)
prior <- 0.5
likelihood <- 1
posterior <- ( (1 - likelihood) / (1 - likelihood * prior ) ) * prior
posterior

prior <- seq(from = 0, to = 1, length.out = 101)
likelihood <- seq(from = 0, to = 1, length.out = 101)
grid <- crossing(prior, likelihood)
grid$posterior <- ( (1 - likelihood) / (1 - likelihood * prior ) ) * prior

grid %>%
  na.omit %>%
  filter(prior %in% c(0.4, 0.6, 0.8) ) %>%
  ggplot(aes(x = likelihood, y = posterior / prior, colour = as.factor(prior) ) ) +
  geom_line() +
  theme_minimal() +
