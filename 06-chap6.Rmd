...
plotly::export(g, file = "assets/mermaid_ch6.pdf")
```
...
ICC <- data.frame(
    Est = ICC_temp,
    Lower = attributes(ICC_temp)$hdi.icc$icc_Participant[1],
    Upper = attributes(ICC_temp)$hdi.icc$icc_Participant[2]
    ) %>%
    round(., 3)
...
```{r postdist, eval = FALSE, fig.pos = "H", fig.width = 5, fig.height = 5, fig.cap = "Posterior distribution of the interaction parameter between Sesson and Condition. The mean and the 95\\% credible interval are displayed at the top and the bottom of the histogram. The green text indicates the proportion of the distribution that is either below or above zero."}
...
```{r plotexp1, fig.pos = "H", fig.width = 8, fig.height = 6, fig.cap = "Mean RUM score by Session and Condition, along with violin plots and individual data. Error bars represent 95\\% CIs."}
